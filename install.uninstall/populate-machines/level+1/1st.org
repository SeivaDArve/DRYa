# Title:       | Dependencies 1st
# Description: | Groups of softwares for each situation (presets): DRYa dependencies are one Preset. There are presets for Linux, Windows, Termux, Live USB OS, configs for softwares, etc.
# Use:         | `D iu presets` and choose from a list. (all lists should be in this file, listed as text to allow manual install also). Also, after a fresh install of DRYa, the 1st preset is also listed as `D iu dp`
:PROPERTIES:
#+STARTUP: overview
#+STARTUP: hidestars
#+STARTUP: indent
...
- [ ] Tem script correspondente a todo este texto. Se sim: `D iu exemplo ...`
  
** f_gr_1_installing_terminal (1)
   ...
   echo "uDev: Instalar terminal Base. Including git"

   # Git is needed to install here. in termux if we delete ~/../usr then all termux content is lost while DRYa is at ~/Repositories/ (to fix termux, just restart and ~/../usr will be automatically created. But all pakages previously installed by termux were lost. Git is needed here because DRYa might need an update before installing further 

** f_gr_2_hard_dependencies (2)
   ...
   less          \  # Dependency for ezGIT (example: `git diff --staged` will only list diferences if less is installed. It will claim 'pager' was not found
   vim           \  # Most Powerfull text editor
   nvim          \  # neovim suporta melhor o clipboard
   file          \  # Dependency for ezGIT
   fzf           \  # Fuzzyfinder (`D iu i fzf` fornece a instalacao oficial dos desenvolvedores do fzf)
   -y

   # uDev: Bash deve ter metodos para evitar usar `file` e assim `file` deixava de ser uma dependencia do ezGIT

** f_gr_3_soft_DRYa_dependencies (3)
...
** f_usefull (4)
   ...
   # List of usefull apps 
      ...
      gnupg \        # Package "GNU Privacy Guard" that allows command `gpg` for Encryption/Decryption of all kinds of files with the protocol OpenPGP (protocol used first by the proprietary software PGP "Pretty Good Privacy"
      tar \          # Comprimit pastas com gzip para ficheiros .tar.gz
      ...
      which \          # Used to detect the path of installed scripts
      man \            # Man pages. Software to present manual pages (it does not come default in arch, for example)
      translate-shell  # Software tradutor de idiomas. exemplo: `trans -b :en "Olá, detetando a lingua e passando para en"`
      playwright-cli \ # Headless browser, mais poderoso que puppeteer, util para webcrawling e webscraping. Precisa de node.js
      emacs
      ...
      GParted
      http://strudel.cc  # Live coding para padroes de musica em javascript
      htop           # Task manager com hotkeys para `kill`processos
      tmate          # Similar to TeamViewer or AnyDesk (but to CLI interface with tmux)
      yad            # `apt install yad` para criar system trays com icons e comandos (junto ao relogio)
      iconv          # converte [àáâãä] em [a] nas buscas de texto do `grep` (dependencia de `upk RAL <busca>`)
      -y
** f_GUI_util
Discord
LSL (Linden Scripting Language) > Second Life

** f_ssh

   # List of apps for ssh
      pkg install \
      openssh-client  \    # (debian)
      openssh-clients \    # (red-hat)
      openssh         \    # (arch)
      sshd            \    # (servidor, usado nas 3 distribuicoes)
      -y
...
** f_mention_usefull_python_apps (7)
   # List of usefull 'python' apps
...
** f_list_of_usefull_apps_at_WSL2 (8)
...
** f_after_fresh_windows_format (10)
#    - Depois de formatar o windows:
#      - window update
#      - Resolucao
#      - Ativar windows
#      - Language pack (se preciso)
#      - Ponto de restauro do sistema
#      - drivers
#      - Desativar updates automaticos (permanentemente, com software)
#      - Ponto de restauro do sistema novamente
#      - impressoras
#      - browser
#      - winRAR, winZIP
#      - Photoshop
...
** f_create_bootable_windows_USB__CMD_tutorial
- [ ] Tem script correspondente a todo este texto. Se sim: `D create-windows-bootable-USB-cmd`
uDev
