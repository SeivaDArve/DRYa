...
** DRYa acronym (name meaning)
Don't Repeat Yourself (app)

** Name origin
"DRY" is a Coding style. And the goal is to be applied here very heavily. 

** Description
Portable Operative System
Executable Database of solutions: for bugs, automatic instalations, automatic configs)
Terminal based app (TUI) with shortcuts of every day life tasks
...
*** Step 1 - Instaling dependencies first:
`sudo apt install git fzf`
*** Step 2 - Clone DRYa spefically to "$HOME/Repositories/DRYa" folder                    
(optional): change destination to other folder or else it will be cloned exactly to the directory where your prompt is currently at

`git clone https://github.com/SeivaDArve/DRYa.git $HOME/Repositories/DRYa`
*** Step 3 - Installing DRYa 
 `bash $HOME/Repositories/DRYa/install.uninstall/linux-or-WSL/master-bashrc/1-installer.sh` 
